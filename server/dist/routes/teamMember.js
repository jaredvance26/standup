...
const teamMember_1 = require("../services/teamMember");
const auth_1 = require("../middleware/auth");
const router = express_1.default.Router();
const teamMemberService = new teamMember_1.TeamMemberService();
...
const validatePhotoDataUrl = (photoDataUrl) => {
...
const uploadPhotoToCloudinary = async (photoDataUrl, userId) => {
    ...
    const result = await cloudinary_1.v2.uploader.upload(photoDataUrl, {
...
const deleteCloudinaryPhoto = async (photoPublicId) => {
    if (!photoPublicId) {
        return;
    }
    try {
        await cloudinary_1.v2.uploader.destroy(photoPublicId, { resource_type: "image" });
    }
    catch (error) {
...
router.get("/user/:userId/team-members", auth_1.authenticateToken, auth_1.requireOwnUserIdFromParams, async (req, res) => {
    try {
...
router.post("/team-member", auth_1.authenticateToken, auth_1.requireOwnUserIdFromBody, async (req, res) => {
    let uploadedPhotoPublicId = null;
    try {
        const { firstName, lastName, position, jiraId, userId, photoDataUrl } = req.body;
        if (!firstName || !lastName || !userId) {
            return res.status(400).json({
                error: "Missing required fields. Please provide firstName, lastName, and userId",
            });
        }
        let photoUrl = null;
...
router.put("/team-member/:id", auth_1.authenticateToken, async (req, res) => {
    let uploadedNewPhotoPublicId = null;
    try {
        const updateData = {};
        const { firstName, lastName, position, jiraId, userId, photoDataUrl, photoRemoved, } = req.body;
        const existingTeamMember = await teamMemberService.getTeamMemberById(req.params.id);
        ...
        if (typeof jiraId !== "undefined")
            updateData.jiraId = jiraId;
        ...
        if (typeof photoDataUrl === "string" && photoDataUrl.trim()) {
            const uploadedPhoto = await uploadPhotoToCloudinary(photoDataUrl, photoFolderUserId);
        ...
        const shouldDeleteOldPhoto = oldPhotoPublicId &&
            (photoRemoved ||
...
router.delete("/team-member/:id", auth_1.authenticateToken, async (req, res) => {
    try {
        const existingTeamMember = await teamMemberService.getTeamMemberById(req.params.id);
        if (!existingTeamMember) {
            return res.status(404).json({ error: "Team member not found" });
        ...
        const deletedTeamMember = await teamMemberService.deleteTeamMember(req.params.id);
